...
class WeatherAnalyzer:
    ...
    def _round_value(
        self, value: Optional[float], decimals: int = 1
    ) -> Optional[float]:
    ...
    def _analyze_hourly_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "precipitation_analysis": {
                "total_precipitation": self._round_value(
                    next_24h["precipitation"].sum()
                ),
                "precipitation_hours": len(next_24h[next_24h["precipitation"] > 0]),
                "max_precipitation_rate": self._round_value(
                    next_24h["precipitation"].max()
                ),
                "precipitation_probability_avg": self._round_value(
                    next_24h["precipitation_probability"].mean(), 0
                ),
                "heavy_precipitation_hours": len(
                    next_24h[next_24h["precipitation"] > 2.5]
                ),
            },
            "solar_analysis": {
                "total_sunshine_hours": self._round_value(
                    next_24h["sunshine_duration"].sum() / 3600
                ),
                "peak_radiation": self._round_value(
                    next_24h["shortwave_radiation"].max()
                ),
                "solar_energy_total": self._round_value(
                    next_24h["shortwave_radiation"].sum()
                ),
    ...
    def _analyze_daily_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary_stats": {
                "total_precipitation": self._round_value(
                    next_7_days["precipitation_sum"].sum()
                ),
                "total_sunshine_hours": self._round_value(
                    next_7_days["sunshine_duration"].sum() / 3600
                ),
                "avg_uv_index": self._round_value(
                    next_7_days["uv_index_max"].mean(), 1
                ),
            },
            "daily_breakdown": [
                {
                    ...
                    "daylight_hours": self._round_value(
                        row.get("daylight_duration", 0) / 3600
                    ),
                    ...
                    "precipitation": {
                        ...
                        "probability": self._round_value(
                            row.get("precipitation_probability_max"), 0
                        ),
                        "hours": self._round_value(row.get("precipitation_hours"), 0),
                    },
                    "solar": {
                        "sunshine_hours": self._round_value(
                            row.get("sunshine_duration", 0) / 3600
                        ),
                        "uv_index_max": self._round_value(row.get("uv_index_max"), 1),
                        "solar_energy": self._round_value(
                            row.get("shortwave_radiation_sum")
                        ),
                    },
                    "wind": {
                        "dominant_direction": self._round_value(
                            row.get("wind_direction_10m_dominant"), 0
                        )
                    },
            ...
            "best_days": {
                "sunniest_day": self._find_extreme_day(
                    next_7_days, "sunshine_duration", "max"
                ),
                "rainiest_day": self._find_extreme_day(
                    next_7_days, "precipitation_sum", "max"
                ),
            },
        }
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        if current_temp > 25:
            summary["key_highlights"].append(
                f"Warm temperatures today ({self._round_value(current_temp)}°C)"
            )
        if current_precip > 0:
            summary["key_highlights"].append(
                f"Precipitation: {self._round_value(current_precip)}mm"
            )
        ...
        return [
            {
                ...
                "precipitation": {
                    "probability": self._round_value(
                        row.get("precipitation_probability"), 0
                    ),
    ...
    def _find_extreme_day(
        self, daily_data: pd.DataFrame, column: str, extreme_type: str
    ) -> Dict:
