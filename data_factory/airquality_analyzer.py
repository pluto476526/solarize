...
class AirQualityAnalyzer:
        ...
        return {
            "aqi_indices": {
                "european_aqi": {
                    ...
                    "category": self._get_european_aqi_category(current.get("european_aqi")),
            ...
            "other_indicators": {
                "aerosol_optical_depth": self._round_value(current.get("aerosol_optical_depth")),
    ...
    def _analyze_hourly_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "pollutant_trends": {
                ...
                "nitrogen_dioxide": self._analyze_pollutant_trend(next_24h, "nitrogen_dioxide"),
            },
            "peak_periods": {
    ...
    def _analyze_aqi_data(self) -> Dict[str, Any]:
        ...
        hourly_aqi_estimates = [self._calculate_aqi_from_pollutants(row) for _, row in next_24h.iterrows()]
    ...
    def _analyze_pollutants(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "gaseous_pollutants": {
                "ozone": {
                    "value": self._round_value(current.get("ozone")),
                    "health_impact": self._get_ozone_health_impact(current.get("ozone")),
                    "sources": "Photochemical reactions from NOx and VOCs"
                },
                "nitrogen_dioxide": {
                    "value": self._round_value(current.get("nitrogen_dioxide")),
                    "health_impact": self._get_no2_health_impact(current.get("nitrogen_dioxide")),
                    "sources": "Fuel combustion, vehicle emissions"
                },
                "sulphur_dioxide": {
                    "value": self._round_value(current.get("sulphur_dioxide")),
                    "health_impact": self._get_so2_health_impact(current.get("sulphur_dioxide")),
                    "sources": "Burning fossil fuels, industrial processes"
                },
                "carbon_monoxide": {
                    "value": self._round_value(current.get("carbon_monoxide")),
                    "health_impact": self._get_co_health_impact(current.get("carbon_monoxide")),
    ...
    def _generate_health_recommendations(self) -> Dict[str, Any]:
        ...
        if pm25 > 35.4:
            ...
            recommendations["general_population"].append("Reduce prolonged outdoor exertion")
        elif pm25 > 12:
            recommendations["sensitive_groups"].append("Reduce outdoor activities")
        
        # PM10 based recommendations
        if pm10 > 154:
            recommendations["outdoor_activities"].append("Consider rescheduling outdoor events")
        
        # Ozone based recommendations
        if ozone > 100:
            recommendations["sensitive_groups"].append("Avoid afternoon outdoor activities")
            recommendations["outdoor_activities"].append("Schedule activities for morning hours")
        
        # General recommendations
        if pm25 > 12 or pm10 > 50:
            recommendations["indoor_air_quality"].append("Use air purifiers with HEPA filters")
            recommendations["indoor_air_quality"].append("Keep windows closed during high pollution hours")
        
        # UV index recommendations
        uv_index = current.get("uv_index", 0)
        if uv_index > 6:
            recommendations["outdoor_activities"].append("Use sun protection - high UV levels")
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        # Add key findings
        if pm25 > 12:
            summary["key_findings"].append(f"Elevated PM2.5 levels ({self._round_value(pm25)} μg/m³)")
        if pm10 > 50:
            summary["key_findings"].append(f"Elevated PM10 levels ({self._round_value(pm10)} μg/m³)")
    ...
    def _get_european_aqi_category(self, aqi: Optional[float]) -> str:
        ...
        if aqi is None or pd.isna(aqi):
            return "UNKNOWN"
        aqi = float(aqi)
        if aqi <= 25: return "VERY_GOOD"
        elif aqi <= 50: return "GOOD"
        elif aqi <= 75: return "MODERATE"
        elif aqi <= 100: return "POOR"
    ...
    def _get_us_aqi_category(self, aqi: Optional[float]) -> str:
        ...
        elif aqi <= 150: return "UNHEALTHY_FOR_SENSITIVE_GROUPS"
    ...
    def _get_pm25_category(self, pm25: Optional[float]) -> str:
        ...
        elif pm25 <= 55.4: return "UNHEALTHY_FOR_SENSITIVE_GROUPS"
        elif pm25 <= 150.4: return "UNHEALTHY"
        elif pm25 <= 250.4: return "VERY_UNHEALTHY"
        else: return "HAZARDOUS"
    ...
    def _get_pm10_category(self, pm10: Optional[float]) -> str:
        ...
        elif pm10 <= 254: return "UNHEALTHY_FOR_SENSITIVE_GROUPS"
        elif pm10 <= 354: return "UNHEALTHY"
        elif pm10 <= 424: return "VERY_UNHEALTHY"
        else: return "HAZARDOUS"
    ...
    def _analyze_pollutant_trend(self, hourly_data: pd.DataFrame, pollutant: str) -> Dict[str, Any]:
        ...
        return {
            "trend": trend,
            "change_percent": self._round_value(change_percent, 1)
        }

    def _find_worst_air_quality_period(self, hourly_data: pd.DataFrame) -> Dict[str, Any]:
    ...
    def _find_best_air_quality_period(self, hourly_data: pd.DataFrame) -> Dict[str, Any]:
    ...
    def _find_air_quality_improvement_period(self, hourly_data: pd.DataFrame) -> Dict[str, Any]:
    ...
    def _get_pm25_health_impact(self, pm25: Optional[float]) -> str:
        if pm25 is None or pd.isna(pm25): return "Unknown"
        pm25 = float(pm25)
        if pm25 <= 12: return "Low health risk"
        elif pm25 <= 35.4: return "Moderate risk - Unusual sensitivity possible"
        elif pm25 <= 55.4: return "Increased risk for sensitive groups"
        elif pm25 <= 150.4: return "Health alert - Everyone may experience effects"
        else: return "Health warnings of emergency conditions"

    def _get_pm10_health_impact(self, pm10: Optional[float]) -> str:
        if pm10 is None or pd.isna(pm10): return "Unknown"
        pm10 = float(pm10)
        if pm10 <= 50: return "Low health risk"
        elif pm10 <= 154: return "Moderate risk"
        elif pm10 <= 254: return "Unhealthy for sensitive groups"
        else: return "Health alert"

    def _get_ozone_health_impact(self, ozone: Optional[float]) -> str:
        if ozone is None or pd.isna(ozone): return "Unknown"
        ozone = float(ozone)
        if ozone <= 50: return "Good"
        elif ozone <= 100: return "Moderate"
        else: return "Poor - Respiratory irritation possible"

    def _get_no2_health_impact(self, no2: Optional[float]) -> str:
        if no2 is None or pd.isna(no2): return "Unknown"
        no2 = float(no2)
        if no2 <= 50: return "Good"
        elif no2 <= 100: return "Moderate"
        else: return "Poor - Respiratory effects"

    def _get_so2_health_impact(self, so2: Optional[float]) -> str:
        if so2 is None or pd.isna(so2): return "Unknown"
        so2 = float(so2)
        if so2 <= 50: return "Good"
        elif so2 <= 100: return "Moderate"
        else: return "Poor - Respiratory irritation"

    def _get_co_health_impact(self, co: Optional[float]) -> str:
        if co is None or pd.isna(co): return "Unknown"
        co = float(co)
        if co <= 5000: return "Good"
        elif co <= 10000: return "Moderate"
        else: return "Poor - Potential health effects"
