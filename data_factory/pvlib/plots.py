...
def solar_elevation_chart(solar):
    ...
    fig.update_layout(
        title="Solar Elevation vs Time", xaxis_title="Time", yaxis_title="Elevation (°)"
    )
    return chart(fig)
...
def sunpath_chart(solar):
    ...
    fig = go.Figure(
        go.Scatterpolar(
            r=sample["elevation"],
            theta=sample["azimuth"],
            mode="markers",
            marker=dict(size=3),
        )
    )
    fig.update_layout(
        title="Sunpath Diagram", polar=dict(radialaxis=dict(title="Elevation (°)"))
    )
    return chart(fig)
...
def poa_vs_ghi_chart(irradiance, weather):
    fig = go.Figure(
        go.Scatter(
            x=irradiance["poa_global"], y=weather["ghi"], mode="markers", opacity=0.5
        )
    )
    fig.update_layout(
        title="POA vs GHI", xaxis_title="POA (W/m²)", yaxis_title="GHI (W/m²)"
    )
    return chart(fig)
...
def irradiance_breakdown_chart(weather):
    ...
    for comp in ["ghi", "dni", "dhi"]:
        if comp in weekly_weather.columns:
            fig.add_trace(
                go.Scatter(
                    x=weekly_weather.index,
                    y=weekly_weather[comp],
                    stackgroup="one",
                    name=comp.upper(),
                )
            )
    fig.update_layout(
        title="Irradiance Breakdown (GHI / DNI / DHI)",
        xaxis_title="Time",
        yaxis_title="W/m²",
    )
    return chart(fig)
...
def poa_heatmap(irradiance):
    ...
    fig.update_layout(
        title="POA Irradiance Heatmap (Day vs Hour)",
        xaxis_title="Hour",
...
def temp_wind_chart(weather):
    ...
    fig.add_trace(
        go.Scatter(
            x=weekly_weather.index, y=weekly_weather["temp_air"], name="Temp (°C)"
        )
    )
    fig.add_trace(
        go.Scatter(
            x=weekly_weather.index,
            y=weekly_weather["wind_speed"],
            name="Wind (m/s)",
            yaxis="y2",
        )
    )
...
def temp_vs_irradiance(cell_temp, irradiance):
    ...
    fig.add_trace(
        go.Scatter(
            x=irradiance["poa_global"],
            y=cell_temp["temperature"],
            mode="markers",
            name="",
            opacity=0.5,
        )
    ...
    fig.update_layout(
        title="Module Temperature vs Irradiance",
...
def temp_derating(cell_temp, dc):
    ...
    fig.add_trace(
        go.Scatter(
            x=cell_temp["temperature"], y=dc["p_mp"], mode="markers", opacity=0.5
        )
    )
    fig.update_layout(
        title="Temperature Derating Impact",
        xaxis_title="Cell Temp (°C)",
...
def dc_vs_irradiance(dc, irr):
    fig = go.Figure()
    fig.add_trace(
        go.Scatter(x=irr["poa_global"], y=dc["p_mp"], mode="markers", opacity=0.5)
    )
    fig.update_layout(
        title="DC Power vs Irradiance",
        xaxis_title="POA (W/m²)",
        yaxis_title="DC Power (W)",
    )
    return chart(fig)
...
def dc_vs_ac(dc, ac):
    fig = go.Figure()
    fig.add_trace(
        go.Scatter(x=dc["p_mp"], y=ac["ac"], mode="markers", name="DC/AC", opacity=0.5)
    )
    fig.update_layout(
        title="DC vs AC Power", xaxis_title="DC Power (W)", yaxis_title="AC Power (W)"
    )
    return chart(fig)
...
def inverter_efficiency(dc, ac):
    ...
    fig.add_trace(
        go.Scatter(x=dc["p_mp"], y=efficiency, mode="markers", name="", opacity=0.4)
    )
    fig.update_layout(
        title="Inverter Efficiency Curve",
        xaxis_title="DC Power (W)",
...
def monthly_yield(ac):
    ...
    fig.update_layout(
        title="Monthly Energy Yield", xaxis_title="Month", yaxis_title="Energy (kWh)"
    )
    return chart(fig)
...
def power_heatmap(ac):
    ...
    fig.update_layout(
        title="Power Output Heatmap (Day vs Hour)",
        xaxis_title="Hour",
        yaxis_title="Day of Year",
    )
    return chart(fig)
...
def peak_power_vs_irradiance(ac, irradiance):
    ...
    fig.update_layout(
        title="Daily Peak Power vs Irradiance",
        xaxis_title="Avg Irradiance (W/m²)",
        yaxis_title="Peak AC Power (W)",
    )
    return chart(fig)
...
def daily_yield(ac):
    ...
    fig.update_layout(
        title="Daily Energy Yield", xaxis_title="Date", yaxis_title="Energy (kWh)"
    )
    return chart(fig)
...
def cumulative_energy(ac):
    ...
    fig.update_layout(
        title="Cumulative Energy (kWh)", xaxis_title="Time", yaxis_title="kWh"
    )
    return chart(fig)
...
def performance_ratio(ac, irradiance):
    ...
    fig.update_layout(
        title="Performance Ratio (PR) over Time", xaxis_title="Time", yaxis_title="PR"
    )
    return chart(fig)
