...
data['temp_effect'] = data['temp'] * data['sun_elevation'].clip(0)  # Temp matters more when sun is high
...
features = [
    'temp', 'humidity', 'cloud_cover', 'wind_speed',
    'sun_elevation', 'sun_azimuth', 'hour', 'dayofyear',
    'month', 'temp_effect', 'cloud_sun_interaction'
]

X = data[features]
y = data['ac_energy']  # Direct target - learn to predict PVWatts output from env factors
...
print(f"Environment-based range: [{env_based_pred.min():.2f}, {env_based_pred.max():.2f}]")
