        ...
        try:
            ...
            with self.db.cursor() as cur:
                psycopg2.extras.execute_batch(
                    cur, query, records, page_size=self.page_size
                )
    ...
    def save_modelchain_result(
        ...
        with self.db.cursor() as cur:
                ...
                for idx, d in enumerate(dfs):
                    ...
                    for col in d.columns:
                        if pd.api.types.is_numeric_dtype(
                            d[col]
                        ) or pd.api.types.is_bool_dtype(d[col]):
                            d[col] = d[col].apply(
                                lambda x: (
                                    None
                                    if pd.isna(x)
                    ...
                    cols = ["result_id", "utc_time", "array_name"] + [
                        c
                        for c in d.columns
                        if c not in ("result_id", "utc_time", "array_name")
                    ]
            ...
            if hasattr(result, "aoi") and result.aoi is not None:
                aoi_tuple = (
                    result.aoi if isinstance(result.aoi, tuple) else (result.aoi,)
                )
            ...
            if hasattr(result, "airmass") and result.airmass is not None:
                dfs = (
                    result.airmass
                    if isinstance(result.airmass, tuple)
                    else (result.airmass,)
                )
                insert_timeseries(dfs, "airmass")

            if (
                hasattr(result, "cell_temperature")
                and result.cell_temperature is not None
            ):
                dfs = (
                    tuple(
                        ct.to_frame(name="temperature")
                        for ct in result.cell_temperature
                    )
                    if isinstance(result.cell_temperature, tuple)
                    else (result.cell_temperature.to_frame(name="temperature"),)
                )
            ...
            if hasattr(result, "diode_params") and result.diode_params is not None:
                dfs = (
                    result.diode_params
                    if isinstance(result.diode_params, tuple)
                    else (result.diode_params,)
                )
            ...
            if hasattr(result, "total_irrad") and result.total_irrad is not None:
                dfs = (
                    result.total_irrad
                    if isinstance(result.total_irrad, tuple)
            ...
            if hasattr(result, "solar_position") and result.solar_position is not None:
                dfs = (
                    ...
                    if isinstance(result.solar_position, tuple)
                    else (result.solar_position,)
                )
            ...
            if hasattr(result, "weather") and result.weather is not None:
                dfs = (
                    result.weather
                    if isinstance(result.weather, tuple)
                    else (result.weather,)
                )
    ...
    def fetch_modelchain_result(self, result_id):
        ...
        with self.db.cursor() as cur:
            ...
            def fetch_timeseries(table_name):
                ...
                df = pd.read_sql(
                    f"SELECT * FROM {table_name} WHERE result_id=%s ORDER BY utc_time, array_name",
                    engine,
                    params=(result_id,),
                )
                if df.empty:
                ...
                array_groups = [
                    g.drop(columns=["result_id", "array_name"])
                    for _, g in df.groupby("array_name")
                ]
        ...
        with self.db.cursor() as cur:
            ...
            """,
                (
                    location_id,
                    pd.to_datetime(row["time"]).to_pydatetime(),
                    safe_float(row["european_aqi"]),
                    safe_float(row["us_aqi"]),
                    safe_float(row["pm10"]),
                    safe_float(row["pm2_5"]),
                    safe_float(row["carbon_monoxide"]),
                    safe_float(row["nitrogen_dioxide"]),
                    safe_float(row["sulphur_dioxide"]),
                    safe_float(row["ozone"]),
                    safe_float(row["aerosol_optical_depth"]),
                    safe_float(row["dust"]),
            ...
            if not hourly_df.empty:
                hourly_tuples = [
                    (
                        ...
                        pd.to_datetime(r["date"]).to_pydatetime(),
                        safe_float(r["pm2_5"]),
                        safe_float(r["carbon_monoxide"]),
                        safe_float(r["carbon_dioxide"]),
                        safe_float(r["nitrogen_dioxide"]),
                        safe_float(r["sulphur_dioxide"]),
                        safe_float(r["ozone"]),
                        safe_float(r["dust"]),
                        safe_float(r["uv_index"]),
                        safe_float(r["pm10"]),
                    )
                    for _, r in hourly_df.iterrows()
                ]
