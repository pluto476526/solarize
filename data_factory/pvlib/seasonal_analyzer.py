...
class SeasonalAnalyzer:
    ...
    def calculate_monthly_production(self) -> pd.Series:
        ...
        if self.hourly_energy_kwh.index.tz is None:
            self.hourly_energy_kwh.index = pd.to_datetime(
                self.hourly_energy_kwh.index
            ).tz_localize("UTC")
    ...
    def calculate_seasonal_variation(self) -> Dict:
        ...
        if max_production > 0:
            variation_percent = (
                (max_production - min_production) / max_production
            ) * 100
        else:
            variation_percent = 0
        ...
        return {
            ...
            "level": level,
            "max_min_ratio": (
                round(max_production / min_production, 2)
                if min_production > 0
                else float("inf")
            ),
        }
    ...
    def get_seasonal_distribution(self) -> Dict[str, float]:
        ...
        for month, production in monthly_production.items():
            ...
            seasonal_production[season] = (
                seasonal_production.get(season, 0) + production
            )
        ...
        if total_annual > 0:
            return {
                season: round((prod / total_annual) * 100, 1)
    ...
    def get_seasonal_insights(self) -> List[str]:
        ...
        insights.append(
            f"Peak production occurs in {dominant_season[0]} ({dominant_season[1]}% of annual output)"
        )
        insights.append(
            f"Lowest production in {weakest_season[0]} ({weakest_season[1]}% of annual output)"
        )

        if variation["level"] == "low":
            insights.append(
                "Seasonal consistency is excellent with minimal variation throughout the year"
            )
        elif variation["level"] == "moderate":
            insights.append(
                "Moderate seasonal variation - system performs well across most seasons"
            )
        else:
            insights.append(
                "High seasonal variation - consider this for energy storage planning"
            )
        ...
        if summer_percent > winter_percent * 1.5:
            insights.append(
                "System shows strong summer performance characteristic of this location"
            )
    ...
    def generate_seasonal_report(self) -> Dict:
        ...
        return {
            ...
            "monthly_production": {
                month: round(prod, 1)
                for month, prod in self.calculate_monthly_production().items()
            },
        }
